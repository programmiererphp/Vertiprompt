...
4.  **PROJECT OVERVIEW (In Slice 2)**: The second slice you generate must begin with 'Extend [AppName] with following feature slice:' followed by '# Goal' section with a brief, one-sentence stating the slice's specific goal. Example: "# Goal\\nThis slice focuses on implementing the file upload interface."

5.  **INCREMENTAL INSTRUCTIONS (Slice 3+)**: every subsequent slice (Slice 3 and onwards) must start with "Extend the '[AppName]' with following feature slice:"  with following '# Goal' section. This ensures a clear, evolutionary path.
